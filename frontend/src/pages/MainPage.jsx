...
function MainPage() {
  ...
  const loadLinks = useCallback(async (categoryId = null) => {
    try {
      ...
      setError('There was a problem loading links.');
      console.error(err);
    }
  }, []);
  ...
  const handleSave = async (event) => {
    ...
    try {
      ...
      setError('AI save failed. Please try again.');
      console.error(err);
    } finally {
      setSaving(false);
    }
  ...
  const handleMemoChange = async (id, memo) => {
    try {
      ...
      setError('There was a problem saving the memo.');
      console.error(err);
    }
  };
  ...
  const handleLinkCategoryChange = async (id, categoryId) => {
    try {
      ...
      setError('There was a problem updating the link category.');
      console.error(err);
      throw err;
    }
  };
  ...
  return (
    <div className="app">
      <header className="app__header">
        <h1>Smart Link Library</h1>
        <p>Let AI suggest titles and categories automatically.</p>
      </header>
      <main>
        ...
        <section className="link-list">
          {links.length === 0 && (
            <p className="empty-state">No links saved yet. Try adding one.</p>
          )}
          {links.map((link) => (
            <LinkCard
              key={link.id}
